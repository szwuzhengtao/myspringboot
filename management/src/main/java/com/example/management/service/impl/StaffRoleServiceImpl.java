...
public class StaffRoleServiceImpl extends ServiceImpl<StaffRoleMapper, StaffRole> implements StaffRoleService {
    ...
    @Override
    public CommonResult deleteStaffRole(StaffRole staffRole) {
        QueryWrapper wrapper = new QueryWrapper();
        wrapper.eq("staffId",staffRole.getStaffId());
        wrapper.eq("roleId",staffRole.getRoleId());
        staffRoleMapper.delete(wrapper);
        return CommonResult.success();
    }
}
